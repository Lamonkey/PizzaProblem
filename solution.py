...
def selectTeam(numPizza,ingreds,teams,pizzas):
    ...
    while (teams[0] >0 or teams[1] > 0 or teams[2] > 0):
        ...
        pool = []
        p1Flag,p2Flag,p3Flag = False,False,False
        if(teams[0] >0 and 2 <= len(pizzas)):
            p1Flag = True
            p1 = mp.Process(target=calculateScore(2,ingreds,pizzas,result))
            p1.start()
            pool.append(p1)
            #tmpReuslt1,pizzas1,perfectSocre1 = calculateScore(2,ingreds,pizzas)
        if(teams[1] >0 and 3 <= len(pizzas)):
            p2Flag = True
            p2 = mp.Process(target=calculateScore(3,ingreds,pizzas,result))
            p2.start()
            pool.append(p2)
            #tmpReuslt2,pizzas2,perfectSocre2 = calculateScore(3,ingreds,pizzas)
        if(teams[2] >0 and 4 <= len(pizzas)):
            p3Flag = True
            p3 = mp.Process(target=calculateScore(4,ingreds,pizzas,result))
            p3.start()
            pool.append(p3)
        ...
        if(p1Flag):
            result1 = result.get()
            tmpReuslt1,pizzas1,perfectSocre1 = result1["ret"],result1["dupPizza"],result1["perfectScore"]
        if(p2Flag):
            result2 = result.get()
            tmpReuslt2,pizzas2,perfectSocre2 = result2["ret"],result2["dupPizza"],result2["perfectScore"]
        if(p3Flag):
            result3 = result.get()
            tmpReuslt3,pizzas3,perfectSocre3 = result3["ret"],result3["dupPizza"],result3["perfectScore"]
...
def calculateScore(numberOfPeople, ingreds, pizzas,result):
    outDict = {}
    ret = []
    perfectSore = 0
    ...
    outDict["ret"] = ret
    outDict["dupPizza"] = dupPizza
    outDict["perfectScore"] = perfectSore
    result.put(outDict)
